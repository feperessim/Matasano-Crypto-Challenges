...
(define (untemper x)
  (let ((n 624)
	...
	(y 0)
	(tmp 0))
    (set! y (logxor x (ash x (- l))))
    (set! y (logxor y (logand (ash y t) c)))
    (set! tmp (logxor y (logand (ash y s) b)))
    (set! tmp (logxor y (logand (ash tmp s) b)))
    (set! tmp (logxor y (logand (ash tmp s) b)))
    (set! y (logxor y (logand (ash tmp s) b)))
    (set! tmp (logxor y (ash y (- u))))
    (set! y (logxor y (ash tmp (- u))))
    y))


(define (MT19937 seed sixty-four-bits MT)
...
(define n 624)
(define seed 47)
(define random-proc (MT19937 seed #f '()))
(define random-numbers (map (lambda (x) (random-proc)) (iota n)))
(define state-array (map untemper random-numbers))
(define random-proc-clone (MT19937 seed #f state-array))
(define random-numbers (map (lambda (x) (random-proc)) (iota n)))
(define random-numbers-clone (map (lambda (x) (random-proc-clone)) (iota n)))
(display (equal? random-numbers random-numbers-clone))
